...
# df = df.groupby(['period', 'name_fam_last_first'])['total_hours'].sum().to_frame().reset_index()
# df['cum_sum'] = (df.groupby('name_fam_last_first')['total_hours'].cumsum())
df2 = df.groupby('project_code', as_index=False).agg(total_hours=("total_hours", "sum"))
